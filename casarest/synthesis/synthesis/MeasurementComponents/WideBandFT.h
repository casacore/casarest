//# WideBandFT.h: Definition for WideBandFT
...
//#
//#
//# $Id: WideBandFT.h,v 1.2 2006/10/16 01:30:37 mmarquar Exp $

#ifndef SYNTHESIS_WIDEBANDFT_H
#define SYNTHESIS_WIDEBANDFT_H
...
//
// <etymology>
// FTMachine is a Machine for Fourier Transforms. WideBandFT does
// Grid-based Fourier transforms.
// </etymology>
//
// <synopsis> 
// The <linkto class=SkyEquation>SkyEquation</linkto> needs to be able
// to perform Fourier transforms on visibility data. WideBandFT
...
// (e.g. UVW coordinates).
//
// Gridding and degridding in WideBandFT are performed using a
...
class WideBandFT : public FTMachine {
public:
  ...
  // <group>
  WideBandFT(Long cachesize, Int tilesize, String convType="SF",
	 Float padding=1.0, Bool usezero=True);
  WideBandFT(Long cachesize, Int tilesize, String convType,
  ...
  // Construct from a Record containing the WideBandFT state
  WideBandFT(const RecordInterface& stateRec);

  // Copy constructor
  WideBandFT(const WideBandFT &other);

  // Assignment operator
  WideBandFT &operator=(const WideBandFT &other);
...
//		       const VisBuffer& vb);
  ...
  void initializeToSky(ImageInterface<Complex>& iimage, 
		   Matrix<Float>& weight,
		       const VisBuffer& vb);
  //void wbinitializeToSky(ImageInterface<Complex>& iimage, 
//		  ImageInterface<Complex>& iimage1, Matrix<Float>& weight,
//		       const VisBuffer& vb);
  void wbinitializeToSky(ImageInterface<Complex>& iimage, 
		  PtrBlock<ImageInterface<Complex>* > & iimagespec, 
  ...
  // Save and restore the WideBandFT to and from a record
  ...
  Bool modifyWeights(const VisBuffer& vb, Matrix<Float> &specwts, Float power);
  Bool modifySpecModelVis(const VisBuffer& vb, Cube<Complex> &specvis, Float power);
...
protected:
  ...
  PtrBlock<ImageInterface<Complex>* > imagespec;
  PtrBlock<Array<Complex>* > griddedDataspec;
  PtrBlock<Lattice<Complex>* > latticespec;
  PtrBlock<Lattice<Complex> * > arrayLatticespec;
