...
//#
//#
//# $Id: CalDescColumns.h,v 19.5 2004/11/30 17:50:11 ddebonis Exp $
...
class ROCalDescColumns 
{
 ...
 protected:
  ...
  void attach (const CalTable& calTable, ROTableMeasColumn& tabCol, 
	       MSCalEnums::colDef colEnum, const Bool& optional = False);
  void attach (const CalTable& calTable, ROArrayQuantColumn<Double>& tabCol, 
	       MSCalEnums::colDef colEnum, const Bool& optional = False);
...
class CalDescColumns 
{
 ...
 protected:
  ...
  // Return a CalTable cal_desc subtable as a Table reference. 
  // Utilizes friendship relationship with class CalTable.
  Table& calDescAsTable(CalTable& calTable) {return calTable.calDescAsTable();}

  // Attach a table column accessor
  void attach (CalTable& calTable, ROTableColumn& tabCol, 
	       MSCalEnums::colDef colEnum, const Bool& optional = False);
  void attach (CalTable& calTable, ROTableMeasColumn& tabCol, 
	       MSCalEnums::colDef colEnum, const Bool& optional = False);
  void attach (CalTable& calTable, ROArrayQuantColumn<Double>& tabCol, 
	       MSCalEnums::colDef colEnum, const Bool& optional = False);
