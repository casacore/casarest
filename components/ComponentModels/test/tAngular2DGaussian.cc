...
int main() {
	try {
		...
		try {
			// bogus units
			majAx = Quantity(4, "m");
			Angular2DGaussian beam3(majAx, minAx, pa);
		...
		minAx = majAx;
		beam = Angular2DGaussian(majAx, minAx, pa);
		...
		beam2 = Angular2DGaussian::fromRecord(rec);
		...
		v[1] = minAx;
		v[2] = pa;
		Angular2DGaussian beam3(v);
		AlwaysAssert(beam3 == beam, AipsError);

		v = beam.toVector();
		Angular2DGaussian beam4(v);
